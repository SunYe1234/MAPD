			...
			if (targetElement.isPlace()) {
				...
				try {
					...
					if (petriNet.isEnabled(((GraphicTransition) targetElement).getTransition())) {
						petriNetAdapter.getTransition(((GraphicTransition) targetElement).getTransition().getId()).setAvailable(true);
						if (!petriNetAdapter.hasAvailableTrans(((GraphicTransition) targetElement).getTransition().getId()))
								petriNetAdapter.addAvailableTransition(((GraphicTransition) targetElement).getTransition().getId());

						this.canvas.getHighlightedElements().add(targetElement);
						targetElement.setHighlightColor(Colors.PERMITTED);
						this.canvas.repaint();
					} else {
						petriNetAdapter.getTransition(((GraphicTransition) targetElement).getTransition().getId()).setAvailable(false);
						this.canvas.getHighlightedElements().add(targetElement);
