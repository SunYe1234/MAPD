...
import org.pneditor.petrinet.model.entities.ViderArc;
import org.pneditor.petrinet.model.entities.ZeroArc;
		...
		if (this.isNotReset) {
			...
			try {
				...
				if (this.oldArc.getSource().isPlace()) {
					...
					ViderArc viderArc=new ViderArc(arc.getWeight());
					viderArc.setConnectedPlace(arc.getConnectedPlace());
					viderArc.setIn(arc.isIn());
					int transactionId = this.oldArc.getSource().isPlace()?this.oldArc.getDestination().getId():this.oldArc.getSource().getId();
					adapter.replaceArc(arc, viderArc,transactionId);
