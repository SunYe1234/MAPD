			...
			try {
				if (arc.getArc().getMultiplicity() != multiplicity) {
					if (multiplicity < 1) {
						...
						if (!arc.getArc().getDestination().isPlace())
						{
							for (Transition t: petriNetAdapter.getTransitions())
							{
								for (Arc a:t.getInArcs())
								{
									if (a.getConnectedPlace().getId()==arc.getArc().getSource().getId())
										a.setWeight(multiplicity);
								}
